...
class ProductVariantController extends Controller
{
    ...
    {
        $variant = ProductVariant::findOrFail($id);
        $variantItemCheck = ProductVarientItem::where('product_variant_id', $variant->id)->count();
        if($variantItemCheck > 0)
        {
            return response(['status' => 'error', 'message' => 'Can not Delete! Delete Variant Items first to delete this variant!']);
        }
