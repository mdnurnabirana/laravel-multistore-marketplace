...
class VendorController extends Controller
{
    public function dashboard()
    {
        ...
        $totalCompletedOrders = Order::where('order_status', 'completed')->whereHas('orderProducts', function ($query) {
            $query->where('vendor_id', Auth::user()->vendor->id);
        })->count();
        $totalProducts = Product::where('vendor_id', Auth::user()->vendor->id)->count();
        return view('vendor.dashboard.dashboard', 
        compact('todaysOrders', 'todaysPendingOrders', 'totalOrders', 'totalPendingOrders', 'totalCompletedOrders', 'totalProducts'));
    }
}
